...
int main(int argc, char*args[])
{
	...
	do
	{
		...
		selectRC = select(myRcvSocket+1, &readfds,&writefd, NULL, &timeout);
		...
		if (list.size>0 && FD_ISSET(myRcvSocket,&writefd)) 
		{
			client_t *q=slist_pop_first(&list);
			toUpperCase(q->buffer);
			sendto(myRcvSocket, q->buffer, strlen(q->buffer), 0,(struct sockaddr *) &(q->clientAddr), sizeof(clientAddr));	
		}  
...
int isPosNumeric(const char *str) // check if the word contains only positive digits
{
    ...
    int num= atoi(str);
    if(num <1 || num > 65535) // if port num are not correct
    	return FALSE;
